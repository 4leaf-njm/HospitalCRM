...
<link rel="stylesheet"
	href='<c:url value="/resources/css/commons/memberJoin.css"/>'>

<section class="content">
	<form class="join_box" action="memberJoin" method="post">

		<h2 class="form-signin-heading">회원 정보 입력</h2>
		<legend></legend>
		
		<div class="row">
			<div class="col-md-2">
				<label for="mem_pcode">환자번호</label><span class="star">＊</span>
			</div>
			<input type="text" class="form-control" id="mem_pcode" name="mem_pcode" readonly="readonly" />
		</div>
		<br />
		
		<div class="row">
			<div class="col-md-2">
				<label for="mem_name">이름</label> <span class="star">＊</span>
			</div>
			
			<input type="text" class="form-control" id="mem_name" name="mem_name" maxlength="20"
				value='' /> <span class="tip-txt" style="color: blue">이름이
				부정확할 경우 진료예약 및 조회가 불가능합니다.</span>
		...
		<div class="row">
			<div class="col-md-2">
				<label for="mem_birth">주민번호</label> <span class="star">＊</span>
			</div>
			<input type="text" class="form-control" id="mem_birth2" name="mem_birth2" maxlength="6"
				value='' />&nbsp;-&nbsp;<input type="password" name="mem_birth3" id="mem_birth3"
				 maxlength="7" value='' class="form-control"/>
		...
		<div class="row">
			<div class="col-md-2">
				<label for="mem_id">아이디</label> <span class="star">*</span>
			</div>
			<input type="text" id="mem_id" name="mem_id" class="form-control" maxlength="20" />
			<button type="button" id="chkId" class="btn btn-primary" onclick="chkId();" style="margin-left: 10px; display: inline-block;">중복확인</button>
			<button type="button" id="resetId" class="btn btn-info" style="margin-left: 10px; display: none;">다시입력</button>
			<span class="tip-txt" id="chkmsg"style="color: blue">6~20자이여야 하며,
				한글/특수문자는 입력이 불가능합니다.</span>
			<p class="validation-txt" id="messageUserId" style="display: none;"></p>
		</div>
		<br />

		<div class="row">
			<div class="col-md-2">
				<label for="mem_pwd">비밀번호</label> <span class="star">*</span>
			</div>
			<input type="password" class="form-control" id="mem_pwd" name="mem_pwd" maxlength="20" />
		...
		<div class="row">
			<div class="col-md-2">
				<label for="mem_pwd_chk">비밀번호 확인</label> <span class="star">*</span>
			</div>
			<input type="password" class="form-control" id="mem_pwd_chk" name="mem_pwd_chk"
		...
		<div class="row">
			<div class="col-md-2">
				<label for="mem_phone">휴대폰</label> <span class="star">*</span>
			</div>
			<select class="form-control three" id="mem_phone1" name="mem_phone1" title="휴대폰 첫번째 자리 입력">
			...
			</select> &nbsp;-&nbsp; <input type="text" class="form-control three" id="mem_phone2" name="mem_phone2" maxlength="4" />
			&nbsp;-&nbsp; <input type="text" class="form-control three" id="mem_phone3" name="mem_phone3" maxlength="4" />
			<input type="hidden" name="mem_phone" />
			<p class="tip-txt" style="color: blue">※ 예약 관련정보가 발송됩니다. 올바른 번호를
				입력하세요.</p>
		</div>
		<br />
		...
		<div class="row">
			<div class="col-md-2">
				<label for="mem_email">이메일</label> <span class="star">*</span>
			</div>
			<input type="hidden" name="mem_email" />
			<input type="text" id="mem_email1" class="form-control two" name="mem_email1" maxlength="20">&nbsp;@&nbsp;
			<input type="text" id="mem_email2" class="form-control two" name="mem_email2" maxlength="20"
				readonly="readonly" /> <select name="mem_email3" class="form-control two"
		...
		<div class="row">
			<div class="col-md-2">
				<label for="mem_addr">주소</label> <span class="star">*</span>
			</div>
			<input type="hidden" name="mem_addr" />
			<input type="text" id="mem_addr_number" class="form-control two" name="mem_addr_number" placeholder="우편번호"> <input
			...
			<input type="text" class="form-control" id="mem_addr2" name="mem_addr2"
				placeholder="주소"> <input type="text" class="form-control" name="mem_addr3"
				id="mem_addr3" placeholder="상세주소">
		</div>
		<div style="overflow: scroll; width: 820px; height: 400px; margin-top: 30px; padding: 10px; background-color: #F9F9F9; border: 1px solid #BDBDBD;">
			<p>
				<section class="agree-section">
					<section class="agree-item agree-item-100">
						...
						<div id="SRVC_AGREE_CONT" class="agree-txt scrollable">
							...
							<h1>제1장 총칙 제1조 (목적)</h1>
							<br />
							<p>삼성서울병원(이하 &quot;병원&quot;이라 합니다)에서 운영하는 인터넷 홈페이지에서 제공하는 인터넷
								관련 서비스(이하 &quot;서비스&quot;라 합니다)의 이용에 관한 사항을 규정함을 목적으로 합니다.</p>
							<br />
							<h2>제2조 (정의)</h2>
							<ol>
		...
		<div class="input-checkbox-group" style="margin: 10px 0;">
			...
			<input type="checkbox" id="chk" name="chk" 
				...
				<span class="label-text">위 이용 약관에 동의합니다.</span>
			</label>
		</div>
		<legend></legend>
		<button class="btn btn-lg btn-primary " onclick="mem_join(this.form);">
		...
		<div class="vertical-alignment-helper">
			<div class="modal-dialog vertical-align-center">
				<div class="modal-content panel-success">
					...
					<div class="modal-body">
						<p class="modal-message">사용가능한 아이디입니다.</p>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-primary btn_use" data-dismiss="modal">사용</button>
						<button type="button" class="btn btn-warning" data-dismiss="modal">다시 입력</button>
					</div>
				</div> 
			</div>
		</div>
	...
	<div class="modal fade" id="dangerModal" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="vertical-alignment-helper">
			<div class="modal-dialog vertical-align-center">
				<div class="modal-content panel-danger">
					...
					<div class="modal-body">
						<p class="modal-message">사용 불가능한 아이디입니다.</p>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">확인</button>
					</div>
		...
		new daum.Postcode(
				{
					oncomplete : function(data) {
						...
						document.getElementById('mem_addr_number').value = data.zonecode; //5자리 새우편번호 사용
						document.getElementById('mem_addr2').value = fullAddr;
...
<script>
	function mem_join(frm) {
		
		$("#mem_phone1 option:selected").val();
		...
		if (frm.mem_name.value == "") {
			...
			frm.mem_name.focus();
			return false;
		} else if (frm.mem_birth2.value == "" || frm.mem_birth3.value == "") {
			alert("주민번호를 입력하세요.");
			frm.mem_birth2.focus();
			frm.mem_birth3.focus();
			return false;
			...
			frm.mem_pwd.focus();
			return false;
		} else if (frm.mem_pwd_chk.value == "") {
			alert("비밀번호를 체크하세요.");
			frm.mem_pwd_chk.focus();
			return false;
		} else if (frm.mem_pwd.value != frm.mem_pwd_chk.value) {
			alert("비밀번호가 같지않습니다.");
			return false;
		} else if (frm.mem_phone2.value == "" || frm.mem_phone3.value == "") {
			alert("휴대폰 번호를 입력하세요.");
			frm.mem_phone2.focus();
			frm.mem_phone3.focus();
			return false;
		} else if (frm.mem_email1.value == "" || frm.mem_email2.value == "") {
			alert("이메일를 입력하세요.");
			frm.mem_email1.focus();
			frm.mem_email2.focus();
			return false;
		} else if (frm.mem_addr_number.value == "" || frm.mem_addr2.value == ""
				|| frm.mem_addr3.value == "") {
			alert("주소를 입력하세요.");
			frm.mem_addr_number.focus();
			frm.mem_addr2.focus();
			frm.mem_addr3.focus();
			return false;
		} else if(!chk){
		...
		$('#successModal .modal-message').text('회원가입에 성공했습니다.');
	...
	$(document).ready(function(){
		$('#mem_pcode').val(Math.floor(Math.random() *(99999999 - 10000000)+ 10000000));
	});
</script>
...
<script>
	$("#chkId").on("click",function() {
		var id = $('#mem_id').val();
		if(id == '') {
			$('#warningModal .modal-message').text('아이디를 입력해주세요.');
		...
		$.ajax({
			...
			success:function(data){
				if(data == 'SUCCESS'){
					$('#successModal .btn_use').click(function(){
						$('#mem_id').attr('readonly', 'readonly');
						$('#chkId').hide();
						$('#resetId').css('display', 'inline-block');;
					});
					$('#successModal').modal('show');
				} else {
